...
export class Dialog extends DialogConfig {
  ...
  public async sync() {
    ...
    const segments = [new HKIDN({segNo: 3, blz, name, systemId: "0"}), new HKVVB({segNo: 4, productId: this.productId, lang: 0}), new HKSYN({segNo: 5}),];
  ...
  public async init() {
    ...
    if (this.useSCA) {
      segments.push(new HKFTAN({segNo: 5, version: 6, process: "4"}));
    }
