...
export abstract class Client {
  ...
  public async balance(account: SEPAAccount): Promise<Balance> {
    ...
    do {
      const request = this.createRequest(dialog, [new HKSAL({
        segNo: 3, version: dialog.hisalsVersion, account, touchdown,
      }),]);
    ...
    const hisal: HISAL = segments.find(s => s.account.accountNumber === account.accountNumber && s.account.blz === account.blz);
    return {
      account,
  ...
  public async standingOrders(account: SEPAAccount): Promise<StandingOrder[]> {
    ...
    const responses: Response[] = [];
    do {
      const request = this.createRequest(dialog, [new HKCDB({
        segNo: 3, version: dialog.hicdbVersion, account, painFormats: dialog.painFormats, touchdown,
      }),]);
      const response = await dialog.send(request);
      touchdowns = response.getTouchdowns(request);
